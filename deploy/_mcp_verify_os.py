...
if not getattr(s, "_surreal_patched", False):
    # Some register paths set the flag; force-patch if needed
    try:
...
if ad.get("compose_style") != "ART_DECO" or os_genome.genome_family(ad) != "ArtDeco":
    ...
    if banned & deco_types:
        print(f"  !! FAIL: ART_DECO banned arch_types present: {banned & deco_types}")
        all_ok = False
    else:
        print("  art_deco_lobby_v1 + ART_DECO tower-ban compose: OK")
