...
try:
    ...
    if len(gnc_objs) < 3:
        ...
        all_ok = False
    persian_spec = GRAPH_REGISTRY["PERSIAN_IWAN"]["spec"]
    persian_objs = spawn_graph(bpy.context, s, persian_spec[:3], spacing=10.0, graph_id="PERSIAN_IWAN")
    print(f"  spawn_graph PERSIAN_IWAN partial: {len(persian_objs)} objects")
    if len(persian_objs) < 2:
        print(f"  !! FAIL: PERSIAN_IWAN spawn got {len(persian_objs)}")
        all_ok = False
    banned = {"TOWER", "TESSELLATION_TOWER", "BELL_TOWER", "WATCHTOWER", "OBELISK", "KEEP"}
    persian_types = {row[0] if isinstance(row, (list, tuple)) else row.get("arch_type") for row in persian_spec}
    if banned & persian_types:
        print(f"  !! FAIL: PERSIAN_IWAN banned arch types: {banned & persian_types}")
        all_ok = False
    else:
        print("  PERSIAN_IWAN banned arch_type set empty: OK")
...
if not xf:
    ...
    all_ok = False
elif "PERSIAN_IWAN" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing PERSIAN_IWAN")
    all_ok = False
elif "ZEN_STREAM_GARDEN" not in (xf.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing ZEN_STREAM_GARDEN")
    all_ok = False
else:
    print(f"  axis_compression type={xf.get('type')} BRUTALIST_PLAZA+PERSIAN_IWAN: OK")
...
pi = os_genome.load_genome("persian_iwan_courtyard_v1")
if pi.get("compose_style") != "PERSIAN_IWAN" or os_genome.genome_family(pi) != "Persian":
    print(f"  !! FAIL: persian_iwan_courtyard_v1 compose/family")
    all_ok = False
elif pi.get("grammar_id") != "PERSIAN_IWAN":
    print(f"  !! FAIL: persian_iwan_courtyard_v1 grammar={pi.get('grammar_id')}")
    all_ok = False
elif pi.get("surreal_transform") != "axis_compression":
    print(f"  !! FAIL: persian_iwan transform={pi.get('surreal_transform')}")
    all_ok = False
elif os_styles.get("PERSIAN_IWAN", {}).get("gate") != "_lib_ARCHWAY_ADV":
    print("  !! FAIL: PERSIAN_IWAN compose_roles missing")
    all_ok = False
elif os_styles.get("PERSIAN_IWAN", {}).get("corner_tower") != "_lib_PILLAR":
    print("  !! FAIL: PERSIAN_IWAN corner_tower must be PILLAR")
    all_ok = False
elif pi.get("compose_roles", {}).get("corner_tower") != "_lib_PILLAR":
    print(f"  !! FAIL: persian genome corner_tower override={pi.get('compose_roles')}")
    all_ok = False
else:
    print("  persian_iwan_courtyard_v1 + PERSIAN_IWAN compose_roles: OK")
