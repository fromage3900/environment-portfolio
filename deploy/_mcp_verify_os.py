...
if ad.get("compose_style") != "ART_DECO" or os_genome.genome_family(ad) != "ArtDeco":
    ...
    if banned & deco_types:
        print(f"  !! FAIL: ART_DECO banned arch_types={banned & deco_types}")
        all_ok = False
    else:
        print("  art_deco_lobby_v1 + ART_DECO compose_roles (tower-ban): OK")

# axis_compression must cover ART_DECO after tower-ban rematerialize
_xf_ad = get_transform("axis_compression")
if not _xf_ad or "ART_DECO" not in (_xf_ad.get("applies_to") or []):
    print("  !! FAIL: axis_compression missing ART_DECO")
    all_ok = False
else:
    print("  axis_compression applies_to ART_DECO: OK")
