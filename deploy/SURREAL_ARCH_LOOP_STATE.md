...
- **`meso_pyramid_courtyard_v1`** genome + **`MESOAMERICAN_PYRAMID`** grammar (retaining tiers, ceremonial stair, colonnade, ROMAN portal, sacred pool)
- **`MESOAMERICAN_PYRAMID`** compose style — `corner_tower` → `_lib_PILLAR` (tower-ban); library bake for retaining wall + stair block
- Research preset `meso_pyramid_courtyard_graph` + curated playable preset; `axis_compression` transform
- World manifest embed verify; genome catalog **31**; ROUND→ROMAN hygiene
...
- Broader tower-ban sweep (Gothic / Romanesque / Renaissance / Moorish corner_tower → PILLAR)
