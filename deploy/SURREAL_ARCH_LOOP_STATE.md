...
- **`ART_DECO`** grammar rematerialized — facade bay, panel wall, filigree, cusped portal, balcony, ceremonial ramp (no TESSELLATION_TOWER / OBELISK)
- Compose roles: `large`→BAROQUE_FACADE, `corner_tower`→PILLAR, `monument`→PUBLIC_FOUNTAIN
- Genome `axis_compression` + corner_tower override; research study `research/artdeco/01_lobby_promenade.md`
- Library bake for **`GREYBOX_RAMP`**; OS/world verify assert banned arch types empty

## Next loop targets

- Mesoamerican pyramid courtyard full wire (stub genome+grammar present; ROUND→ROMAN)
