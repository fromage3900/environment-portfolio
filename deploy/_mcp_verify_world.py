...
try:
    ...
    meso_pyramid_export_root = mproot
    s._active_style_genome = None
    print(f"  meso_pyramid_compose: {mpmsg} metrics={verify_hooks.compose_metrics(mproot)}")
    if mproot.get("surreal_style_genome_id") != "meso_pyramid_courtyard_v1":
        print(f"  !! FAIL: meso_pyramid genome stamp got {mproot.get('surreal_style_genome_id')}")
        all_ok = False
    else:
        print("  meso_pyramid_compose: OK")
except Exception as e:
    print(f"  meso_pyramid compose error: {e}")
    all_ok = False
...
try:
    ...
    if meso_pyramid_export_root is not None:
        mpm = export.build_world_manifest(meso_pyramid_export_root, monolith=s)
        ...
        if mpsg.get("resolved_compose_roles", {}).get("gate") != "_lib_ARCHWAY_ADV":
        ...
        if mpsg.get("resolved_compose_roles", {}).get("sacred") != "_lib_GREYBOX_STAIR_BLOCK":
