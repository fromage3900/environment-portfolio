...
try:
    ...
    meso_plan = plans.spawn_village_plan(location=(156, 0, 0))
    ...
    if mesoroot.get("surreal_style_genome_id") != "meso_pyramid_courtyard_v1":
        print(f"  !! FAIL: meso genome stamp got {mesoroot.get('surreal_style_genome_id')}")
        all_ok = False
    else:
...
try:
    ...
    if meso_export_root is not None:
        ...
        if msg.get("resolved_compose_roles", {}).get("corner_tower") != "_lib_PILLAR":
            raise RuntimeError("MESOAMERICAN_PYRAMID corner_tower must be PILLAR (no towers)")
